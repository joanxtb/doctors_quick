...
const DoctorListItem: React.FC<DoctorListItemProps> = ({ doctor, onBook }) => {
    return (
        <div className="col-md-6 col-sm-12 col-lg-4 p-0" role="listitem">
            <figure aria-label={`book ${doctor.name} ${doctor.lastname}`} className="card p-2 m-1" style={{ background: `url(${doctor.photo}) no-repeat left center white`, overflow: 'hidden', border: '0px', height: 120, backgroundSize: '120px' }}>
